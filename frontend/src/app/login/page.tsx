...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err?.response?.status === 429) setErro("Muitas tentativas. Aguarde.");
  ...
  const handleGoogle = async () => {
    ...
    try {
      const result = await signIn("google", { redirect: false, callbackUrl: "/auth/google-callback" });
    ...
    <div className="w-full max-w-sm relative">
      <div className="rounded-2xl p-8" style={{ background: "#111827", border: "1px solid #1f2937" }}>
        ...
        <button onClick={handleGoogle} disabled={loadingGoogle}
          className="w-full flex items-center justify-center gap-3 py-2.5 rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50 mb-4"
